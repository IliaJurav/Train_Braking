...
Модуль главного окна,
 также известного как окно ввода данных
...
#---------------------------------------------------------------------------
class mainWindow:
    # запуск браузера со страницей о локомотиве
    def open_web(self):
        if len(self.wwwLoko)>0:
            webbrowser.open_new_tab(self.wwwLoko)
    #---------------------------------------------------------------------------
    # конструктор
    # params - стуктура dict, ожидаются три ключа
    # 'models': [[txt,class,[pars]]] список доступных методов расчёта
    # 'connect': cnx connection к базе данных
    # 'baseEditor': окно редактирования базы данных
    def __init__(self,params=[['не заданы методы']]):
        ...
        # ------------------------------------------------------------------------
        # переключение панелей при смене выбора метода расчёта
        # функция выталкивает наверх нужные панели для ввода данных
        def cbSelected(event):
            # получаем список нужных панелей
            cur = pans[comboMetod.current()]
            # тащим их наверх
            for p in cur:
                if p.upper() == 'L':
                    fWay.tkraise() # ввод длины пути
                if p.upper() == 'DL':
                    fdWay.tkraise() # ввод точности длины пути
                if p.upper() == 'V':
                    fSpeed.tkraise() # ввод начальной скорости
                if p.upper() == 'DV':
                    fdSpeed.tkraise() # ввод интервала скорости
                if p.upper() == 'DT':
                    fdTime.tkraise() # ввод интервала времени
        # --------------------------------------------------------------------
        # create panel 1
        ...
        # начальные условия
            ...
            # выбираем из таблицы указатель на  класс в соответствии с выбранным методом
            classMetod = params['models'][metod][1]
            # создание экземпляра класса
            data = classMetod(self.paramsTrain,
                              self.paramsLoko, startCondition)

            # выполнение расчётов
            data.calculate()

            # создание окна для вывода результатов расчётов
            rezWin = tk.Tk()
