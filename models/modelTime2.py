...
# wo = 18,6 + 0,08v + 0,0024v^2, (17)
        ...
        x0=[0.0, Vst] #  начальные условия

        rrr  = models.rkf.rkf(f=myF, a=0, b=1000, x0=x0, atol=1e-8, rtol=1e-6,
                       hmax=self.interval, hmin=0.001)
        t, sv = rrr.solve()
        s, v = sv.T
...
# ---------------------------------------------------------------------------
    def putResult2docx(self):
        ...
        models.write2docx.putDocFile('Расчёт длины тормозного пути поезда\nметод расчёта по интервалам времени c RKF45', tabls, [self.plt_rezult])

def demo():
# начальные условия
    startCondition = pd.DataFrame([[68.0, -10.0, 1, 0]],
				  columns=['v0','slope','dt','type'])
...
# параметры вагонов
    ...
    data = modelTimeRKF(paramsTrain, paramsLoko, startCondition)
    rez = data.calculate(startCondition['v0'][0])
    print(rez[1][-1],rez[2])
    ...
    return data
    '''
metodDescription = ('Метод расчёта по интервалам времени (RKF45)',
                    modelTimeRKF, ['V','dt'])
