...
замедление в км/ч^2 под действием удельной тормозной силы 1кг/т
 eps = 120 для вагонов, 114 для тепловозов, 107 для электровозов
...
# Расчитываем среднюю скорость на интервале
        ...
        def generatePlot():
            ...
            #colors = ["silver","yellow","limegreen",'#1f77b4',"green","blue","red","brown",
            #          'aqua', "greenyellow", 'gold']
...
def get_demo_obj():
# начальные условия
    startCondition = pd.DataFrame([[68.0, 10, -10, 0]],
				  columns=['v0','step','slope','type'])
...
# параметры вагонов
    ...
    data = modelSpeed(paramsTrain, paramsLoko, startCondition)
    data.calcIntSpeed()
...
def demo():
# начальные условия
    startCondition = pd.DataFrame([[68.0, -10.0, 10, 0]],
				  columns=['v0','slope','step','type'])
# параметры локомотива
    paramsLoko = pd.DataFrame([[138.0, 6, 11.0, 0]],
				  columns=['massa','axles','force','pads'])
