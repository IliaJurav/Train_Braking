# -*- coding: utf-8 -*-
"""
Created on Tue May 21 22:01:24 2024
...
# from dopFunc import getFmt as getFmt
# from dopFunc import CalcPress as CalcPress
# from dopFunc import calcKnt as calcKnt
# from rkf import rkf as rkf
# from write2docx import putDocFile as putDocFile
...
class modelInvertTimeRKF:
    def __init__(self, parTrain, parLoko, extCondition):
        ...
        self.extCondition = extCondition.copy()
        df = pd.DataFrame()
...
# –электровозы и тепловозы на звеньевом пути
# wo = 18,6 + 0,1v + 0,0029v^2, (15)
# –электровозы и тепловозы на бесстыковом пути
# wo = 18,6 + 0,08v + 0,0024v^2, (17)
    ...
    def putReport(self, root):
        ...
        bt2.grid(row=0, column=1,pady=(0, 10),padx=(10, 10))
        fr2.pack()
...
def demo():
# начальные условия
    startCondition = pd.DataFrame([[68.0, -10.0, 1, 0, 0, 25.0, 1900.0, 0.1]],
				  columns=['v0','slope','dt','type','way','air','lenWay','stepL'])
# параметры локомотива
    paramsLoko = pd.DataFrame([[138.0, 6, 11.0, 0]],
				  columns=['massa','axles','force','pads'])

# параметры вагонов
    paramsTrain = pd.DataFrame([[20, 82.0, 4, 7.0, 0],
                                [10, 82.0, 4, 7.0, 0],
                                [20, 82.0, 4, 7.0, 0],
                                ],
	   	      columns=['count','massa','axles','force','pads'])

    data = modelInvertTimeRKF(paramsTrain, paramsLoko, startCondition)
    rez = data.calculate()
    print(rez[1][-1],rez[2][-1])
